...
const registerUser = z.object({
	...
	cpfCnpj: z
		.string()
		.optional()
		.transform((h) => (h ? h.replace(/\D/g, "".trim()) : null)),
...
const updateUser = z.object({
	...
	senha: z
		.string({ message: "senha obrigatório" })
		.optional()
		.transform((h) => (h ? h.trim() : null)),
	photoUrl: z.string().optional(),
	cpfCnpj: z
		.string({ message: "cpf ou cnpj obrigatório" })
		.min(11, "cpf ou cnpj inválido")
		.transform((h) => h.replace(/\D/g, "".trim())),
});
...
const login = z.object({
	...
	senha: z
		.string({ message: "senha obrigatório" })
		.min(6, "Senha deve conter mínimo 6 dígitos"),
});
...
const addTreepycashe = z.object({
	userId: z.string(),
	treepycashe: z.number(),
	florestaId: z.number(),
});
